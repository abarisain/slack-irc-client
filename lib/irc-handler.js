...
var IRC_EVENTS = {
  registered: function registered(context, message) {
    ...
    let options = context.irc.options || {};
    if (options.nickservPassword) {
      context.logger.info('Sending Nickserv password');
      context.irc.client.say(options.nickservBotName || 'nickserv', 'IDENTIFY ' + options.nickservPassword);
    }
  },
