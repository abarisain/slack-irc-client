...
var SLACK_EVENTS = {
  open: function started(context) {
    var team = context.slack.client.data.getTeamById(
      context.slack.client.rtm.activeTeamId
    );
    context.logger.info('Slack: Connected to Slack team "%s"', team.name);

    // Prepare IRC connection
    var user = context.slack.client.data.getUserById(
      context.slack.client.rtm.activeUserId
    );
    var userConfig;
    try {
    ...
    context.irc.options = {
      ...
      nick: userConfig.nick || user.name,
      realName: userConfig.realName || user.realName,
  ...
  message: function message(context, message) {
    ...
    if (!message.subtype) {
      context.logger.verbose(
        'Slack: New channel message in %s: "%s"',
        target,
        message.text
      );
      ...
      context.logger.verbose(
        'Slack: New action message in %s: "%s"',
        target,
        message.text
      );
      ...
      context.logger.verbose(
        "Slack: No handler for message type %s",
        message.subtype
      );
      return;
    }
  ...
  init: function initSlackHandler(context) {
    ...
    if (!context.slack.events) {
      context.logger.error(
        "Cannot bind Slack event handler before Slack event listener"
      );
      return;
    }
    ...
    _.forEach(SLACK_EVENTS, function(handler, eventName) {
      ...
      handler._boundListener = function() {
        try {
          ...
          context.logger.error(
            "Failed to handle Slack event %s.",
            eventName,
            error
          );
        }
